-- Update the parameterless version of get_completed_services_pending_invoices
-- to exclude jobs paid in cash (which already have receipts)
...
AS $function$
  ...
  WHERE j.company_id = public.get_user_company_id()
    ...
    AND (j.payment_method IS NULL OR j.payment_method != 'cash')
...
-- Update the parameterized version of get_completed_services_pending_invoices
-- to also exclude jobs paid in cash
CREATE OR REPLACE FUNCTION public.get_completed_services_pending_invoices(p_company_id uuid)
 RETURNS TABLE(job_id uuid, client_id uuid, client_name text, service_date date, job_type text, duration_minutes integer, cleaner_id uuid, cleaner_name text, completed_at timestamp with time zone)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
