-- Recriar a view financial_ledger com filtros corrigidos
-- Problema: Invoices "Draft" e Cleaner Payments "Pending" estavam aparecendo
-- Solução: Filtrar apenas transações finalizadas

DROP VIEW IF EXISTS public.financial_ledger;

CREATE VIEW public.financial_ledger WITH (security_invoker = true) AS

-- INVOICES: Apenas pagas (status = 'paid')
SELECT 
  i.id,
  i.company_id,
  i.client_id,
  ...
  COALESCE(i.payment_method, (
    ...
    WHERE pr.job_id = i.job_id AND pr.sent_at IS NOT NULL 
    LIMIT 1
...
-- PAYMENT RECEIPTS: Apenas enviados (sent_at IS NOT NULL)
SELECT 
  pr.id,
  pr.company_id,
  pr.client_id,
...
-- CLEANER PAYMENTS: Apenas aprovados ou liquidados
SELECT 
  cp.id,
  cp.company_id,
  j.client_id,
